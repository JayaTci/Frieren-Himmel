...
export function CinematicReveal() {
  ...
  const { videoRef, canvasRef, loaded, loadProgress, loadedRef, seekTo } =
    useScrollDrivenVideo("/source-media/processed-videos/middle.mp4");
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (seqReadoutRef.current) {
          const n = Math.min(169, Math.floor(progress * 169) + 1);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-x-0 top-[36%] z-20 flex flex-col px-6 md:hidden">
          {BEATS.map((b) => {
            const visible = visibleBeats.has(b.id);
            return (
              <div
              ...
              >
                ...
                >
                  ...
                  <figcaption className="mt-3 flex items-center justify-between">
                    ...
                    <span className="font-sans text-[10px] tracking-[0.1em] text-zinc-300">{b.film}</span>
                  </figcaption>
                </figure>
              </div>
            );
